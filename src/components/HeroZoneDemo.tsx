/**
 * HeroZoneDemo — interactive drag-and-drop zone demo for the landing page.
 *
 * Uses framer-motion drag (not dnd-kit) for a lightweight demo experience.
 * Blocks can be dragged between zones; token counts animate on change.
 */

import { useState, useRef, useCallback } from "react"
import { motion } from "framer-motion"
import { springs } from "@/lib/motion"
import { AnimatedNumber } from "@/components/ui/animated/AnimatedNumber"
...
export function HeroZoneDemo() {
  ...
  const handleDragEnd = useCallback(
    (blockId: string, _event: MouseEvent | TouchEvent | PointerEvent, info: { point: { x: number; y: number } }) => {
      ...
      // If dropped outside all zones, dragSnapToOrigin handles snap-back
    },
